	...
	if [ $findCnt -gt 0 ]; then
		...
		for targetFile in "${ES5_TARGET_FILES[@]}"; do
			...
			cp $targetFile ${targetFile}.backup
		        ...
		        sed -i 's/\"https:\/\/basemaps.cartocdn.com\/.*maxZoom:20/\"https:\/\/map.pstatic.net\/nrb\/styles\/basic\/1728003511\/\{z\}\/\{x\}\/\{y\}@2x\.png\?mt\=bg\.ol\.ts\.ar\.lko"\,\{minZoom:6,maxZoom:19/g' $targetFile
			gzip -f -k $targetFile
		done
