...
for targetFile in "${ES5_TARGET_FILES[@]}"; do
	...
	cp $targetFile ${targetFile}.backup
        sed -i 's/\"https:\/\/basemaps.cartocdn.com\/.*maxZoom:20/"https:\/\/map.pstatic.net\/nrb\/styles\/basic\/\{z\}\/\{x\}\/\{y\}\.png\?mt\=bg\.ol\.ts\.ar\.lko",\{minZoom:6,maxZoom:20,continuousWorld:\!0/g' $targetFile
	gzip -f -k $targetFile
done
